...
import de.quist.app.mapsabstraction.model.Polyline;
...
public class PolylineDemoActivity extends FragmentActivity
    ...
    private static final LatLng PERTH = BuildConfig.MAP_IMPLEMENTATION.latLng(-31.95285, 115.85734);

    private static final LatLng LHR = BuildConfig.MAP_IMPLEMENTATION.latLng(51.471547, -0.460052);
    private static final LatLng LAX = BuildConfig.MAP_IMPLEMENTATION.latLng(33.936524, -118.377686);
    private static final LatLng JFK = BuildConfig.MAP_IMPLEMENTATION.latLng(40.641051, -73.777485);
    private static final LatLng AKL = BuildConfig.MAP_IMPLEMENTATION.latLng(-37.006254, 174.783018);
    ...
    public void onMapReady(Map map) {
        ...
        map.addPolyline((BuildConfig.MAP_IMPLEMENTATION.polylineOptions())
        ...
        int radius = 5;
        PolylineOptions options = BuildConfig.MAP_IMPLEMENTATION.polylineOptions()
                .add(BuildConfig.MAP_IMPLEMENTATION.latLng(SYDNEY.latitude() + radius, SYDNEY.longitude() + radius))
                .add(BuildConfig.MAP_IMPLEMENTATION.latLng(SYDNEY.latitude() + radius, SYDNEY.longitude() - radius))
                .add(BuildConfig.MAP_IMPLEMENTATION.latLng(SYDNEY.latitude() - radius, SYDNEY.longitude() - radius))
                .add(BuildConfig.MAP_IMPLEMENTATION.latLng(SYDNEY.latitude() - radius, SYDNEY.longitude() + radius))
                .add(BuildConfig.MAP_IMPLEMENTATION.latLng(SYDNEY.latitude() + radius, SYDNEY.longitude() + radius));
